...
Every spec PR and impl PR opened by the OpenSpec flow agent SHALL contain a usage table in its body, recording which sub-agents and skills were invoked during the agent run. The table is injected by a post-agent workflow step rather than self-reported by the agent.
