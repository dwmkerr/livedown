...
Every spec PR and impl PR opened by the OpenSpec flow agent embeds a
structured usage table in its body that records which sub-agents and
skills were invoked during the run, sourced from the CI session log.
The table is wrapped in HTML comment markers so it can be found and
updated programmatically without affecting the rendered PR body.
