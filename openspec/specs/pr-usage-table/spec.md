...
Every spec PR and impl PR opened by the OpenSpec flow agent SHALL contain
a structured usage table that records which sub-agents and skills were
invoked during the run. The table is injected by a post-agent workflow step
that scrapes the Claude session log after the agent completes.
