...
Every spec PR and impl PR opened by the OpenSpec flow agent SHALL contain a usage table in its body that records the sub-agents and skills invoked during the agent run. The table is populated by scraping the Claude session log using `dwmkerr/claude-toolkit` in a post-agent workflow step.
