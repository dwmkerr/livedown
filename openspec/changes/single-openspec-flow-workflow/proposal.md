...
- Rename the existing job in `openspec-flow.yaml` from `openspec-flow` to `propose` for clarity.
...
- `openspec-flow-unified`: A single GitHub Actions workflow that drives the full OpenSpec lifecycle — propose (explore + build-spec) triggered by issue events, and implement (apply + verify + archive) triggered by proposal PR merge — within one file with shared configuration.
...
- `.github/workflows/openspec-flow.yaml` — expanded with second job and unified env block.
