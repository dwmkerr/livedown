...
- [ ] 1.3 Rename the existing `openspec-flow` job to `plan`.
...
- [ ] 1.5 Update any env var references inside the `implement` job that previously read from the file-level `env:` block to ensure they resolve correctly from the shared top-level `env:` block (no functional change expected — just verify nothing is shadowed).
...
- [ ] 3.4 Confirm the `plan` job's trigger guard still handles only `assigned` and `labeled` issue events.
