...
- A second key — the **view key** — is generated (or supplied via `--view-key`) alongside the existing edit key. The view key is a 64-hex-char random token (32 bytes).
...
- Viewers enter the view key via a new browser modal; the browser sends a `view-auth` message; the relay replies with `view-auth-ack` and full content.
...
- **`src/party/livedown.ts`** — per-connection `viewAuthenticated` flag; new `view-auth` message handler; gated `init` and `update` routing.
- **`public/index.html`** — new view-key modal; `view-auth` / `view-auth-ack` / `view-auth-error` handling; auto-re-auth; profile badge update.
