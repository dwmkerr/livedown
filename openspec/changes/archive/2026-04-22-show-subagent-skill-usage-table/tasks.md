...
- [x] 4.1 Pin the version of `dwmkerr/claude-toolkit` used in both steps to a specific release tag — N/A: `dwmkerr/claude-toolkit` is an interactive Claude Code plugin (uses `grepsession.sh` on local JSONL files) and cannot run in CI. Per CLAUDE.md, the CI implementation uses `jq`/Python on `/home/runner/work/_temp/claude-execution-output.json` directly. No external dependency to pin.
